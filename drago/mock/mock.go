...
func PopulateWithData(repo state.Repository) error {
	...
	repo.UpsertInterface(ctx, &structs.Interface{ID: uuid.Generate(), NodeID: nid, NetworkID: netID, Name: "wg0", Address: "192.168.0.1", ListenPort: 9999})
	repo.UpsertInterface(ctx, &structs.Interface{ID: uuid.Generate(), NodeID: nid, NetworkID: netID, Name: "wg1", Address: "192.168.0.2", ListenPort: 9898})
